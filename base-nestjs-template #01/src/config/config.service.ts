...
import { AzureConfig, ConfigAuthData, ConfigAuthorizationData, ConfigData, ConfigDBData, NeutrinoConfig, PlatformAPIs } from './config.interface';
...
export class ConfigService {
  ...
  private parseNeutrinoConfigFromEnv(env: NodeJS.ProcessEnv, defaultConfig: Readonly<NeutrinoConfig>) {
    return {
  ...
  private parseAzureConfigFromEnv(env: NodeJS.ProcessEnv, defaultConfig: Readonly<AzureConfig>): AzureConfig {
    return {
      searchLogUpload: {
        connectionString: env.AZURE_SEARCH_LOG_STORAGE_CONNECTION_STRING || defaultConfig.searchLogUpload.connectionString,
        containerName: env.AZURE_SEARCH_LOG_STORAGE_CONTAINER_NAME || defaultConfig.searchLogUpload.containerName,
      },
      logoUpload: {
        connectionString: env.AZURE_LOGO_STORAGE_CONNECTION_STRING || defaultConfig.logoUpload.connectionString,
        containerName: env.AZURE_LOGO_STORAGE_CONTAINER_NAME || defaultConfig.logoUpload.containerName,
      },
    };
  }
  private parseAuthorizationConfigFromEnv(env: NodeJS.ProcessEnv): ConfigAuthorizationData {
    return {
  ...
  private parseNotificationsConfigFromEnv(env: NodeJS.ProcessEnv): PlatformAPIs {
    return {
  ...
  private parseAuthConfigFromEnv(env: NodeJS.ProcessEnv): ConfigAuthData {
    let jwksUrl = env.AUTH0_JWKS_URL || env.JWKS_URI || DEFAULT_CONFIG.auth.jwksuri;
    if (!/\/\.well-known\/jwks\.json$/i.test(jwksUrl)) {
      jwksUrl = urlJoin(jwksUrl, '.well-known', 'jwks.json');
    }
    return {
      jwksuri: jwksUrl,
      audience: env.AUTH0_AUDIENCE_URL || env.AUDIENCE || DEFAULT_CONFIG.auth.audience,
      tokenIssuer: env.AUTH0_TOKEN_ISSUER_URL || env.TOKEN_ISSUER || DEFAULT_CONFIG.auth.tokenIssuer,
